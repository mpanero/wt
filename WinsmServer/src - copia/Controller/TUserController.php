...
class TUserController extends AppController
{
	...
	public function login(){
		...
		if($this->request->session()->check('Auth.TUser.token')){
			...
			if($this->request->is('post')) {
				...
				$auth = $this->TUser->find('all', array('conditions' => array('TUser.ACTIVE ' => 1, 'TUser.PASSWORD ' => Security::hash($pass,"SHA256",true), 'TUser.MAIL' => $user)))->first();
                //var_dump($auth);
                //debug($auth);
                ...
                if ($auth) {
                    ...
                    $data = array('TUser' => array('id' => $auth->ID_USER, 'sub' => $auth->ID_USER, 'user' => $auth->MAIL, 'token' => $token));
					// Set Storage
    ...
    {        
        ...
        $email->to($user->MAIL);
        $email->from('smartradeagro@gmail.com');
        ...
        if ($this->request->is('post')) {
            ...
            if($tUser){// si existe tRequest               
                ...
                if ($this->TUser->save($tUser)) {
                    ...
                    $tUser = "Contraseña no Actualizada";
                    $this->set('tUser', $tUser);
                    $this->set('_serialize', ['tUser']);                
                }
