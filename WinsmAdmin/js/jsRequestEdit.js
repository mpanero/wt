...
$(document).ready(function(){    
    $("#pagActual").html("Actualizar Solicitud");
    $("#tipoProducto").off("change").on("change",function(e){
        
        var tProdSelected = $(this).val();
        linksWs["tRequest"].t_product.ID_CATEGORY_PROD = tProdSelected;
        if(tProdSelected != ""){
            if(tProdSelected == 1){
    ...
    var tProdSelected = linksWs["tRequest"].t_product.ID_CATEGORY_PROD;
    getProduct(tProdSelected);
});   

function checkedElementView(){
    if(linksWs["tRequest"].t_product.ID_CATEGORY_PROD != "" && linksWs["tRequest"].t_product.ID_CATEGORY_PROD != 0){
        $("#divProd").prop("style","display:block;");
        $("#divTPri").prop("style","display:block;");
        $("#divPFr").prop("style","display:block;");
        $("#divMone").prop("style","display:block;");
        ...
        if(linksWs["tRequest"].t_product.ID_CATEGORY_PROD >= 1  && linksWs["tRequest"].t_product.ID_CATEGORY_PROD <= 3 ){
        ...
        if(linksWs["tRequest"].t_product.ID_CATEGORY_PROD == 1 ){
            if(linksWs["tRequest"].ID_TYPE_PRICE != 210 ){
                $("#divPrePos").prop("style","display:block;");
                $("#divfech").prop("style","display:block;");
            }else{
                $("#divPrePos").prop("style","display:none;");
        ...
        if(linksWs["tRequest"].ID_TYPE_PRICE == 210){ /* Fijo */
            $("#divfechFijD").prop("style","display:none;");
            $("#divfechFijH").prop("style","display:none;");
            $("#divPreRef").prop("style","display:none;");
            $("#divPos").prop("style","display:none;");
            ...
            if(linksWs["tRequest"].ID_TYPE_PRICE == 211){ /* A fijar */
                $("#divfechFijD").prop("style","display:block;");
                $("#divfechFijH").prop("style","display:block;");
                $("#divPreRef").prop("style","display:block;");
                /* $("#divPos").prop("style","display:block;"); */
        ...
        if(linksWs["tRequest"].ID_PRICE_REF == 220){ /* Chicago */
            $("#divPos").prop("style","display:block;");
        }else{
            $("#divPos").prop("style","display:none;");
        }
...
function loadEventsEditReq(){
    
    /*** add new ***/
    $(".btnRenew").off("click").on("click",function(events){
        blockUISystem();
        $("#ID_USER_OWNER").val(localStorage.getItem('user'));
        if(validacionMaestra("#renewRequest")){
            $('#renewRequest').off("submit").one("submit", function(e){
                //var datosFormEmp = new FormData(this);
                var datosFormEmp = $('#renewRequest').serialize();
                $.ajax( {
                    method: 'post',
                    dataType:"json",
                    url: root+"/TRequest/editRequest.json?"+datosFormEmp, 
                    headers: {
                    ...
                    if (evalResponseAjax(doneNew)) {
                        swal({
                            title: "Mensaje!",
                            text: 'Solicitud Actualizada <span style="color:#4CAF50">Correctamente<span>',
                            type: "success",
                        ...
                        function(isConfirm){
                            if (isConfirm) {
                                linksWs["tRequests"].data[idReq[1]] = doneNew.tRequest;
                                /****************************** Cerrar Modal Edit   *************************/
...
function showRequest(){
    if(linksWs["tRequest"] != null){
        $.each(linksWs["tRequest"], function (atributo, value) { 
        ...
        var tProdSelected = linksWs["tRequest"].t_product.ID_CATEGORY_PROD;  
        $('#tipoProducto option[value="' + tProdSelected + '"]').prop('selected', true);   
        if(tProdSelected != ""){
            if(tProdSelected == 1){
                $("#divCose").prop("style","display:block;");
            }else{
    ...
    findTEntrega("", function(tentrega){
        ...
        if(tentrega.length > 0){
            $.each(tentrega, function (index, entrega) {                
                optRep += ''+
                '<label class="radio-inline">'+
                    '<input type="radio" id="deli_'+entrega.ID_TYPE+'" value="'+entrega.ID_TYPE+'" name="ID_TYPE_DELIVERY" class="styled radioBtn" >'+
                    entrega.INFO+
                '</label>';
    ...
    findProduct(1,tp, function(productos){
        ...
        if(productos.length > 0){
            $.each(productos, function (index, producto) {
                if(linksWs["tRequest"].t_product.ID_PRODUCT == producto.ID_PRODUCT){
                    optRep += '<option value="'+producto.ID_PRODUCT+'" selected>'+producto.PRODUCT_NAME+'</option>';
                }else{
